...
res = duckdb.sql("COPY (SELECT ssn(i) as national_id, clinical_trial_outcome(random()) as clinical_trial_outcome FROM generate_series(1, 100000) s(i)) TO 'data/outcome.parquet'  (FORMAT 'parquet')")
